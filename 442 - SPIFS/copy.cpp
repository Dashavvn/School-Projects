...
bool Filesystem::create(const string name)
{
  ...
  unsigned int d_size = m_disk_size/1052;
  ...
  for(unsigned int i = 0; i < d_size; ++i){ 
	...
	int position = m_dir[j].pos;
	while(position > 0 && position < m_disk_size){
	  if(position > max){
	    max = position;
	  }
	  position = *(int *)&m_disk[position+1024];
	}
      }
      //create the file
      ...
      m_dir[i].size = 0;
      if(MAX_SIZE == 0){
	m_dir[i].pos = (m_disk_size/1052) *24;
      }
      else{
	m_dir[i].pos = MAX_SIZE + 1028;
      }
      return true;
    }
  }
...
bool Filesystem::read(const int fd, string &buffer, const unsigned int size) const
{
  ...
  if(fd>=0 and fd < 255){
    ...
    if(file_list[fd] == NULL){  //file descriptor is invalid or over reads virtual disk
      return false;
    }
    else{
      for(unsigned int i = 0; i < size; ++i){
	++handler->size;
	buffer += m_disk[location+i];
	location = handler->offset + handler->pos;
	if(location%1024==0){
	  int next = *(int *)&m_disk[handler->pos+1024];
	  if(next <= m_disk_size)
	  {
	    handler->pos = next;
	  }
